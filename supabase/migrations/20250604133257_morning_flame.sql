/*
  # Add Stripe Checkout PostgreSQL Function
  
  1. New Functions
    - `create_checkout_session`: PostgreSQL function that acts as a proxy between the frontend and the Stripe checkout Edge Function
      - Parameters:
        - price_id (text): Stripe price ID
        - success_url (text): URL to redirect after successful payment
        - cancel_url (text): URL to redirect after cancelled payment
        - mode (text): Checkout mode (subscription or payment)
        - customer_email (text): Customer's email address
      
  2. Security
    - Function uses SECURITY DEFINER to ensure it runs with elevated privileges
    - JWT token is passed to the Edge Function for authentication
*/
    ...
    FROM
        net.http_post(
            ...
            headers := jsonb_build_object(
                ...
                'Authorization', 'Bearer ' || current_setting('request.jwt.claim.sub', true) -- Pass user ID for authentication
            ),
            body := request_body
        );
